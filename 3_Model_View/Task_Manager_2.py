...
class Task_Manager_2:
    ...
    def __init__(self, window, task_model):
        ...
        ### Modified to share the model between views
        # TK imposes a single window so the display has been modified to appear in a frame in the window
        self.tasks_manager_2_frame = tk.LabelFrame(self.window, text="Task Manager 2", labelanchor='nw')
        self.tasks_manager_2_frame.pack(side="left", fill=tk.BOTH, expand=True)
        ###

        self.entry_var = tk.StringVar(self.tasks_manager_2_frame)
        self.priority_var = tk.StringVar(self.tasks_manager_2_frame)
        ...
        self.tasks_manager_2_frame.bind_all("<MouseWheel>", self._on_canvas_mousewheel)
        ###
    ...
    def _create_the_tasks_main_frame(self):
        ...
        self.tasks_main_frame = tk.Frame(self.tasks_manager_2_frame)
    ...
    def _create_the_new_task_frame(self):
        button = tk.Button(self.tasks_manager_2_frame, text="Add Task", width=10, command=lambda: self.action_pop_up("Add"))
    ...
    def handle_update_task(self, task_id):
        if 0 <= task_id < len(self.tasks_list):
            if len(self.entry_var.get()) > 0:
                ...
                for idx, task in enumerate(self.tasks.read()):
                    if task == selected_task_tuple:
    ...
    def action_pop_up(self, action_var, task_id=None):
        self.popup_window = tk.Toplevel(self.tasks_manager_2_frame)
        ...
        # Configure grid column weights to make the entry expand with tasks_manager_2_frame width
        self.popup_window.grid_columnconfigure(0, weight=1)
        self.popup_window.grid_columnconfigure(1, weight=3)

    def run(self):
        ### Modified to share the model between views
        self.tasks_manager_2_frame.mainloop()
        ###
...
if __name__ == "__main__":
    ...
    ### Modified to share the model between views
    # TK allows different views but imposes a single Tk instance
    # so Task_Manager has been modified to now be displayed in a frame
    window = tk.Tk()
    window.title("Task Manager 2")
