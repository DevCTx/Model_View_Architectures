...
class Task_Manager_2:
    ...
    def __init__(self):
        self.tasks_list: list[(str, str)] = []
        # Fill the list for demonstration purpose
        for i in range(20):
            self.tasks_list.append((f"Task {i + 1}", f"{(i + 2) % 4 + 1}"))
    ...
    def handle_add_task(self):
        if len(self.entry_var.get()) > 0:
            self.tasks_list.append((self.entry_var.get(), self.priority_var.get()))
            self.clear_pop_up_and_input_fields()

    def handle_update_task(self, task_id):
        if len(self.entry_var.get()) > 0:
            if 0 <= task_id < len(self.tasks_list):
                self.tasks_list[task_id] = (self.entry_var.get(), self.priority_var.get())
            self.clear_pop_up_and_input_fields()
